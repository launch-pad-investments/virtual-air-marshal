...
class SpamService(commands.Cog):
            ...
            if support_sys_mng.check_if_support_activated(community_id=ctx.guild.id):
                if await self.send_ticket_message_channel(ctx=ctx, message=message, department='Marketing',
                    ...
                    if await self.send_ticket_message_author(ctx=ctx, time_of_request=time_of_request,
                                                             department='Marketing', ticket_id=ticket_no,
    ...
    async def report(self, ctx, *, message: str):
        ...
        if self.length_checker(message=message):
            ...
            time_of_request = datetime.utcnow()
            if support_sys_mng.check_if_support_activated(community_id=ctx.guild.id):
                if await self.send_ticket_message_channel(ctx=ctx, message=message, department='Report',
                                                          color_code=Colour.purple(), ticket_id=ticket_no,
                                                          time_of_request=time_of_request):
                    if await self.send_ticket_message_author(ctx=ctx, time_of_request=time_of_request,
                                                             department='Marketing', ticket_id=ticket_no,
                                                             support_msg=message, colour=Colour.magenta()):
    ...
    async def general(self, ctx, *, message: str):
        ...
        if self.length_checker(message=message):
            ...
            time_of_request = datetime.utcnow()
            if support_sys_mng.check_if_support_activated(community_id=ctx.guild.id):
                if await self.send_ticket_message_channel(ctx=ctx, message=message, department='General',
                                                          color_code=Colour.orange(), ticket_id=ticket_no,
                                                          time_of_request=time_of_request):
                    if await self.send_ticket_message_author(ctx=ctx, time_of_request=time_of_request,
                                                             department='Marketing', ticket_id=ticket_no,
                                                             support_msg=message, colour=Colour.magenta()):
